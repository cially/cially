	...
	try {
		...
		try {
			...
			for (let i = 0; i < 24; i++) {
				...
				const existingStat = hourlyStats.find(stat => stat.hour === hourString);
			...
			const mondayDate = new Date(today.getTime() - daysFromMonday * 24 * 60 * 60 * 1000);
			...
			for (let i = 0; i < 7; i++) {
				...
				const dayTotal = dayStats.reduce((acc, stat) => ({
					messages: acc.messages + (Number(stat.messages) || 0),
					joins: acc.joins + (Number(stat.joins) || 0),
					leaves: acc.leaves + (Number(stat.leaves) || 0),
					unique_users: Math.max(acc.unique_users, Number(stat.unique_users) || 0) // Take max for unique users
			...
			while (w < 22) {
				...
				const weekTotal = weekStats.reduce((acc, stat) => ({
					...
					joins: acc.joins + (Number(stat.joins) || 0),
					leaves: acc.leaves + (Number(stat.leaves) || 0),
					unique_users: Math.max(acc.unique_users, Number(stat.unique_users) || 0)
				}), { messages: 0, joins: 0, leaves: 0, unique_users: 0 });
