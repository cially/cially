...
async function pbCollectionAutoDelete(guildID) {
	try {
		...
		debug({ text: `Found ${message_records.length} messages to delete for guild ${guildID}` });
		...
		async function bulkDeletion(records) {
			...
			for (let i = 0; i < records.length; i++) {
				...
				if (batchCount < batchSize) {
					...
					try {
						await batch.send();
						debug({ text: `Batch sent successfully. ${totalDeleted} messages deleted so far.` });
					} catch (batchError) {
		...
		debug({ text: `Starting bulk deletion of ${message_records.length} messages...` });
		...
		if (err.status === 404) {
			...
			error({ text: `PocketBase API error during auto-delete for guild ${guildID}:` });
			console.log(`Status: ${err.status}, Message: ${err.message}`);
		} else {
			error({ text: `Unexpected error during auto-delete for guild ${guildID}:` });
			console.log(err);
		}
