...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("scrape-server")
    .setDescription("Scrapes all messages from a server"),
  async execute(interaction) {
    try {
      ...
      if (allMessages.length > 0) {
        ...
        console.log(`\n=== TIME SPAN ===`);
        console.log(
          `Oldest message: ${new Date(oldestTimestamp).toISOString()}`
        );
        console.log(
          `Newest message: ${new Date(newestTimestamp).toISOString()}`
        );
