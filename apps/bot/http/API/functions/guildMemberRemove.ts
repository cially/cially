...
export async function guildMemberRemove(req: Request, res: Response, client?: Client): Promise<Response> {
  const body = req.body;
  ...
  try {
    ...
    if (!collection_name) throw new Error("MEMBER_LEAVES_COLLECTION not configured.");
